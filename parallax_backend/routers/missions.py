...
from parallax_backend.dependencies import RequestIdentity, get_current_identity, require_roles
...
from parallax_backend.schemas import (
    AgentAssessmentRead,
    AgentProposalRead,
    ContextPackRead,
    ErrorResponse,
...
async def create_mission(
    ...
    # K3.3: duplicate/resume resolution. Same operational objective with a
    # verified completion proof -> report the existing mission instead of
    # executing a duplicate. Missing/unverified proof -> create normally.
    fingerprint = task_fingerprint(payload.prompt)
    resolution = await resolve_existing_objective(
        session, identity.workspace.id, project.id, payload.prompt
    )
    if resolution.decision == "completed" and resolution.prior_mission is not None:
        prior = resolution.prior_mission
        record_mission_event(
            session,
            prior,
            "mission.duplicate_recognized",
            "Duplicate objective recognized",
            "A verified completion proof exists; no new mission was created.",
            identity.user.id,
        )
        await session.commit()
        return mission_to_read(prior)
...
@router.post(
    ...
    response_model=MissionRead,
    responses=ERRORS,
    summary="Park a mission until an external event arrives",
)
async def wait_for_event(
    ...
    """K4.3: a mission waiting for an external change consumes no worker.
    ...
    transition_mission(
        ...
        "Parked until an external event arrives",
        identity.user.id,
    )
    record_mission_event(
        session,
        mission,
        "mission.waiting_for_event",
        "Mission parked",
        "Waiting for an external event to wake this mission",
        identity.user.id,
    )
    await session.commit()
    return mission_to_read(await load_mission(session, identity.workspace.id, mission.id))
